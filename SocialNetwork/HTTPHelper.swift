...
struct HTTPHelper {
        ...
        let task = session.dataTaskWithRequest(request) { (data: NSData!, response: NSURLResponse!, error: NSError!) in
            ...
            dispatch_async(dispatch_get_main_queue(), { () -> Void in
                ...
                if httpResponse.statusCode == 200 {
                    ...
                    let errorDict :NSDictionary! = NSJSONSerialization.JSONObjectWithData(data, options: NSJSONReadingOptions.allZeros, error:&jsonerror) as NSDictionary!
    ...
    func getErrorMessage(error: NSError) -> NSString {
        ...
        if error.domain == "HTTPHelperError" {
            ...
            errorMessage = userInfo.description as NSString
        } else {
